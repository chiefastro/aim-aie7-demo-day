...
app = FastAPI(
    title="Street Exeter - Mock Restaurant Server",
    description="Mock web server for Street Exeter restaurant with A2A endpoints",
    version="1.0.0"
)
...
transaction_logic = MockTransactionLogic("street_exeter", data_dir)

# Restaurant info
RESTAURANT_INFO = {
    "name": "Street Exeter",
    "description": "International fusion restaurant serving Korean, Mexican, Vietnamese, and Indian cuisine",
    "address": "8 Clifford Street, Exeter, NH 03833",
    "phone": "(603) 555-0124",
...
async def root():
    ...
    return {
        "restaurant": "Street Exeter",
        "description": "International fusion restaurant",
        "endpoints": {
...
async def health_check():
    ...
    return {"status": "healthy", "restaurant": "Street Exeter"}
...
async def get_osf():
    ...
    if os.path.exists(osf_file):
        with open(osf_file, 'r') as f:
            ...
            osf_data["publisher"]["domain"] = "localhost:8002"
            for offer in osf_data.get("offers", []):
                offer["href"] = offer["href"].replace("localhost:3000", "localhost:8002")
            return osf_data
    else:
...
async def confirm_order(envelope: A2AEnvelope):
    ...
    try:
        ...
        response = A2AConfirmOrderResponse(
            ...
            receipt_url=f"http://localhost:8002/receipt/{order_id}"
        )

...
app = FastAPI(
    title="Newick's Lobster House - Mock Restaurant Server",
    description="Mock web server for Newick's Lobster House restaurant with A2A endpoints",
    version="1.0.0"
)
...
transaction_logic = MockTransactionLogic("newicks_lobster_house", data_dir)

# Restaurant info
RESTAURANT_INFO = {
    "name": "Newick's Lobster House",
    "description": "Classic seafood restaurant serving fresh lobster, fish, and coastal favorites",
...
async def root():
    ...
    return {
        "restaurant": "Newick's Lobster House",
...
async def health_check():
    ...
    return {"status": "healthy", "restaurant": "Newick's Lobster House"}
...
async def get_osf():
    ...
    if os.path.exists(osf_file):
        with open(osf_file, 'r') as f:
            ...
            osf_data["publisher"]["domain"] = "localhost:8003"
            for offer in osf_data.get("offers", []):
                offer["href"] = offer["href"].replace("localhost:3000", "localhost:8003")
            return osf_data
    else:
...
async def confirm_order(envelope: A2AEnvelope):
    ...
    try:
        ...
        response = A2AConfirmOrderResponse(
            ...
            receipt_url=f"http://localhost:8003/receipt/{order_id}"
        )
